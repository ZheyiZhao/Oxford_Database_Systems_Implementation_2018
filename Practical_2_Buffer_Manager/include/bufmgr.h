...
class BufMgr 
{
	private:
        ...
        int head; // added for LRU
        int tail; // added for LRU

	public:
    
		BufMgr( int bufsize );
		~BufMgr();      
		Status PinPage( PageID pid, Page*& page, Bool emptyPage=FALSE ); // *& pass pointer by reference
        ...
        bool FreeSpace(); // added
        Status FreeFrame(int row, int col);// added
